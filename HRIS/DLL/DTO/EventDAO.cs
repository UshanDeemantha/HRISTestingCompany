...
namespace Infox.DTO
{
    public class EventDAO
    {
        ...
        public static List<CalendarEvent> getEvents(int companyId, DateTime start, DateTime end)
        {
        ...
        public static List<CalendarEvent> GetRosterDetailsByRosterGroup(int companyId, int rosterGroupId, DateTime start, DateTime end)
        {
            ...
            try
            {
             ...
             " Time_ShiftTypes.ShiftDescription AS description, Time_DayTypes.DayTypeCode, Time_DayTypes.DayType , " +
             " Time_ShiftSetup.FontColour AS color FROM Time_Roster INNER JOIN Time_ShiftTypes ON Time_Roster.ShiftId = Time_ShiftTypes.ShiftId INNER JOIN Time_DayTypes ON Time_Roster.DayTypeId = Time_DayTypes.DayTypeId" +
             " INNER JOIN Time_ShiftSetup ON Time_ShiftSetup.ShiftId = Time_ShiftTypes.ShiftId"+
             " WHERE Time_Roster.CompanyId=@CompanyId AND RosterGroupId=@RosterGroupId AND Time_Roster.StartTime >= @start AND  Time_Roster.EndTime <= @end ";
        ...
        //this method retrieves all events within range start-end
        public static List<CalendarEvent> GetShifschedulerByEmployee(int companyId, long employeeId, DateTime start, DateTime end)
        {
            ...
            try
            {
                string sqlQuery = "SELECT Time_EmployeeShiftShedule.ShiftSheduleId AS event_id, Time_EmployeeShiftShedule.Subject AS title,"+
                    " Time_ShiftTypes.ShiftDescription AS description,DATEADD(dd, 0, DATEDIFF(dd, 0, Time_EmployeeShiftShedule.StartTime)) AS event_start, "+
                    " DATEADD(dd, 0, DATEDIFF(dd, 0, Time_EmployeeShiftShedule.EndTime)) AS event_end,Time_EmployeeShiftShedule.DayTypeId AS all_day,  "+
                    " Time_ShiftSetup.FontColour AS color " +
                    " FROM Time_EmployeeShiftShedule INNER JOIN Time_ShiftTypes ON Time_EmployeeShiftShedule.ShiftId = Time_ShiftTypes.ShiftId " +
                    " INNER JOIN Time_DayTypes ON Time_EmployeeShiftShedule.DayTypeId = Time_DayTypes.DayTypeId" +
                    " INNER JOIN  Time_ShiftSetup ON Time_ShiftSetup.ShiftId = Time_ShiftTypes.ShiftId"+
                    " WHERE Time_ShiftTypes.CompanyId=@CompanyId AND EmployeeId=@employeeId AND Time_EmployeeShiftShedule.StartTime >= @start AND  Time_EmployeeShiftShedule.EndTime <= @end ";
